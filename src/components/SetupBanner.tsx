...
export function SetupBanner({
  hideDyadProCard = false,
}: {
  hideDyadProCard?: boolean;
} = {}) {
  ...
  const [manualNodePath, setManualNodePath] = useState("");
  const { updateSettings } = useSettings();
  const isWeb = IpcClient.isWebRenderer();
  ...
  const handleManualNodeConfig = useCallback(async () => {
    ...
    try {
      if (isWeb) {
        const trimmed = manualNodePath.trim();
        if (!trimmed) {
          showError("Please enter a Node.js path.");
          return;
        }
        await updateSettings({ customNodePath: trimmed });
  ...
  }, [checkNode, isWeb, manualNodePath, updateSettings]);
                        ...
                        {isWeb ? (
                          <div className="flex flex-col gap-2">
                            <Input
                              ...
                              onChange={(e) => setManualNodePath(e.target.value)}
                              placeholder="/usr/bin/node"
                              className="font-mono text-xs"
                            />
                            <Button
                              onClick={handleManualNodeConfig}
                              disabled={isSelectingPath || !manualNodePath.trim()}
                              variant="outline"
                              size="sm"
                            >
                              {isSelectingPath ? (
