...
### Build Docker image
```bash
# Dockerfile is included in the repo root
docker build -t dyad-web .
```

### Run locally
```bash
docker run -p 4000:4000 dyad-web
```
...
# Install OS deps for git and native modules
RUN apt-get update \
...
```bash
# Use different port
DYAD_WEB_PORT=4001 npm run web:server
...
- Check that `api/ipc/invoke.js` exists
