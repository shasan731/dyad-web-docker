...
- `api/ipc/invoke.js` - Handles POST requests to `/api/ipc/invoke` for IPC calls
- `api/ipc/events.js` - Handles Server-Sent Events (SSE) for event streaming
...
DYAD_DATA_PATH=/mnt/data/dyad-data
NODE_ENV=production
```
...
**Solution**: Ensure the API route exists in `api/ipc/invoke.js` and Vercel deployment includes the `api/` directory.

### Issue: "Database file not found"
**Solution**: In web mode, SQLite is stored under `DYAD_DATA_PATH` on Linux (default: `/mnt/data/dyad-data`). Ensure the directory exists and is writable.
...
# Test IPC
curl -X POST http://localhost:4000/api/ipc/invoke \
